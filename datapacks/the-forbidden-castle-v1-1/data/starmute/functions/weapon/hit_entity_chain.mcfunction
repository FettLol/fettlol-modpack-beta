...
execute at @s positioned ~ ~1 ~ if score #hit in.fire_laser matches 0 if predicate starmute:87_percent if score #distance2 in.fire_laser matches 0..1200 positioned ^ ^ ^0.2 facing entity @e[type=#starmute:mobs,tag=!laser,distance=..10,sort=nearest,limit=1] eyes if block ~ ~ ~ #starmute:airs run function starmute:weapon/ray
